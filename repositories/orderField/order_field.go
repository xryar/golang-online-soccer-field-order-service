...
func (or *OrderFieldRepository) FindByOrderID(ctx context.Context, orderID uint) ([]models.OrderField, error) {
	var orderFields []models.OrderField

	err := or.db.WithContext(ctx).Where("order_id = ?", orderID).Find(&orderFields).Error
	if err != nil {
...
func (or *OrderFieldRepository) Create(ctx context.Context, tx *gorm.DB, request []models.OrderField) error {
